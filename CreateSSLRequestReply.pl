...
# It's used to create standard SECS Request/Reply commands
################################################################################
#!/usr/bin/perl
use strict;
...
my $tempfile = ".\\Info\\SecsCommands.txt";
...
foreach my $line_str (@bucket){
    ...
    if ($line_str =~/^(.*)?\s(.*)$/i) {
        ...
        if($streamFunction =~/S(\d+)F(\d+)/i){
            my $stream = $1;
            my $function = $2;
#            say "Stream: $1, Function: $2";
            if($function != 0 && ($function % 2) != 0){
#                say "Request";
                $requestCmd{"stream"} = $stream;
                $requestCmd{"function"} = $function;
                $requestCmd{"description"} = $description;
            }elsif (%requestCmd) {
#                say "Reply";
                $replyCmd{"stream"} = $stream;
                $replyCmd{"function"} = $function;
                $replyCmd{"description"} = $description;
            }
        }

        if (%requestCmd && %replyCmd){
#            say "Got both";
            createRequestReplyCmds();
            %requestCmd = %replyCmd = ();
        }
    }
}

sub createRequestReplyCmds{
    my $header = '//// '.$requestCmd{"description"}.' ////';
    my $functionName = "S".$requestCmd{"stream"}."F".$requestCmd{"function"}."_Request(){";
    say $header;
    say $functionName;
    say "logn(\"*****    "."S".$requestCmd{"stream"}."F".$requestCmd{"function"}."_".$requestCmd{"description"}."   *****\")".";";
    say "REQUEST.stream = ".$requestCmd{"stream"}.";";
    say "REQUEST.function = ".$requestCmd{"function"}.";";
    say "REQUEST.wbit = 1;";
    say "REQUEST.text = \"\";";
    say "REPLY = wait(SendPrimary(REQUEST, TARGET))".";";
    say "wait( NewTimer( 1 ) )".";	//sleep for 1 sec";
    say "return(REPLY)".";";
    say "}";
    $header = '//// '.$replyCmd{"description"}.' ////';
    my $parameter = "S".$replyCmd{"stream"}."F".$replyCmd{"function"}."_Reply";
    my $parameterText = $parameter.".text";
    $functionName = "S".$replyCmd{"stream"}."F".$replyCmd{"function"}."_Parser(".$parameter."){";
    say $header;
    say $functionName;
    say "logn(\"*****    "."S".$replyCmd{"stream"}."F".$replyCmd{"function"}."_".$replyCmd{"description"}."   *****\")".";";

    say "if(IsMessage(".$parameter.") && ".$parameterText." == \"\")";
    say "return;";

    say "mReplyInfo	= Decode(".$parameterText.")".";";
    say "}";
}
