...
my $tempfile = ".\\Info\\S1F24.txt"; # We can parse file with only content of S1F24

if (!-e $tempfile) {
    say "$tempfile does not exist!";
    $tempfile = ".\\Info\\ASM-Host.log"; # or we can parse for the log file contains content of S1F24
    if (!-e $tempfile) {
        die "$tempfile does not exist!";
    }else {
        extractSECSMessageFromLog("S1F24");
    }
}else{
...
foreach my $line_str (@bucket){
    ...
    if ($isCEIDblock == 0 && $line_str =~/\<L\[3\/1\]/i) {
        #say "CEID Block";
        $isCEIDblock = 1;
        $isRPTblock = 0;
        push(@ceidLines, "CEID", $seperator);
        next;
    }
    ...
    if($isCEIDblock == 1){
        if($isRPTblock == 0){
            if ($line_str =~/\<U4\[1\/1\]\s(\d+)\>/i) {
                say "CEID: $1";
                $ceidDecVal = $1;
                $ceidHexVal = sprintf("%x", $ceidDecVal);
                push(@ceidLines, "\$".$ceidHexVal, $seperator, $ceidDecVal, $seperator);
            }elsif ($line_str =~/\<A\[\d+\/1\]\s"(.*?)"/i) {
                say "CEID Name: $1";
                $ceidDesc = $1;
                push(@ceidLines, $ceidDesc, $seperator);
            ...
            if ($line_str =~/\<U4\[1\/1\]\s(\d+)\>/i) {
                say "RPTID: $1";
                $reportdDecVal = $1;
                $reportdHexVal = sprintf("%x", $reportdDecVal);
                push(@ceidLines, "\tReportID".$seperator,"\$".$reportdHexVal, $seperator, $reportdDecVal, $seperator, "\n");
            }elsif ($line_str =~/\s+\>$/i) {
                #say "End of Report Block";
                $isRPTblock = 0;
                $isCEIDblock = 0;
            }
        }
...
my $outFile = ".\\Info\\EventsAndReports.txt";
say("outFile: ", $outFile);
open(OUTFILE, ">:encoding(UTF-8)", $outFile) || die "ERROR! Can't open $outFile";
print OUTFILE EventsAndReportsHeader(),@ceidLines;
close(OUTFILE) || die "ERROR! Can't close $outFile";

sub EventsAndReportsHeader{
    my $header = '///////////////////////////////////////////////////////////////
// Scripted by THINH NGUYEN
// File: EventsAndReports.txt
// SoftRev: '.$softRev.'
// Date: '.gmtime().'
//
// This file is to configure all ECIDs, and RPTIDs. WARNING!!!
