...
my $logDir =  getcwd() . "\\Logs\\"; #where the interested mes logs are stored to analyze
say "logDir: $logDir";
# my $start_dir = "D:\\Escalation";
# my $top  = MainWindow->new;
# my $logDir = $top->chooseDirectory(-initialdir => $start_dir, -title => 'Choose Project Folder') . "/";
...
#sortLogFilesByTimeStamp(@batchOne, @batchTwo);

if (compareTimeStamp (getLastTimeStamp($logDir . $batchOne[$#batchOne]), getFirstTimeStamp($logDir . $batchOne[$#batchOne]) )) {
...
foreach my $line_str (@logBucket){
    ...
    if ($line_str =~ /^"(\d+\/\d+)","(\d+:\d+:\d+)",(".*?",)"(S\d+F\d+)"(.*)/i) {
        ...
        my $secsCmd = $4;
        my $remain = $5;
        if ($secsCmd eq "S6F11" && $remain =~ /,"(\d+).*",/i) { # We're just interested on events
            my $ceid = $1;
            my $ceidName = "Not Defined!";
            if ($ceidDict{$ceid} ne ""){
                $ceidName = $ceidDict{$ceid};
            }
            my $ceidDetail = "CEID:" . $ceid . " " . $ceidName;
