...
foreach my $line_str (@bucket){
    ...
    chomp($line_str);
    if ($line_str =~/^[\t|\s]+E/i) { # with E notation is Event line
        $eventBegin = 1;
        $reportBegin = 0;
        if ($line_str =~/\s+\$(\w+)\s+\d+\s+\#(.*)/i ||
            $line_str =~/\s+\$(\w+)\s+\d+\s+(.*)/i ) { #Identify CEIDs for ALD
            $ceidHexVal = $1;
            $ceidDesc = $2;
        ...
        if ($line_str =~/\s+\$(\w+)\s+\d+\s+\#(.*)/i) {
            ...
            # }
            push(@ceidLines, "RPID : \$$reportdHexVal : $reportdDecVal : $reportDesc : \n");
        }else{
            say $line_str;
        }
        ...
        if ($reportdHexVal =~/(.*?)\s+\#(.*)/i) {
            $reportdHexVal = $1;
            $reportdDecVal = hex($reportdHexVal);
            #say "\tReportID : \$$reportdHexVal : $reportdDecVal : ";
            push(@ceidLines, "\tReportID : \$$reportdHexVal : $reportdDecVal : \n");
        }else{
            $reportdDecVal = hex($reportdHexVal);
            #say "\tReportID : \$$reportdHexVal : $reportdDecVal : ";
