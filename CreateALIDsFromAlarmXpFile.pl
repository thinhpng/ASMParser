...
foreach my $line_str (@bucket){
    ...
    chomp($line_str);
    if ($line_str =~/\$(\w+)\s+"(.*?)"\s+(#.*)/i) { #Identify CEIDs
        $alidHexVal = $1;
        $alidDecVal = hex($alidHexVal);
        $alidDesc = $2;
        $alidComment = $3;

        $line = "ALID : \$$alidHexVal : $alidDecVal : $alidDesc : $alidComment :\n";
        push(@alidLines, $line);
    }
}

my $outFile = ".\\Info\\ALIDsFromAlarmXp.txt";
say("outFile: ", $outFile);
open(OUTFILE, ">:encoding(UTF-8)", $outFile) || die "ERROR! Can't open $outFile";
print OUTFILE ConvertedEventXpHeader(),@alidLines;
close(OUTFILE) || die "ERROR! Can't close $outFile";

sub ConvertedEventXpHeader{
    my $header = '///////////////////////////////////////////////////////////////
// Scripted by THINH NGUYEN
// File: ALIDsFromAlarmXp.txt
// Version: 1.0
// Date: '.gmtime().'
//
// This file is to configure all ALIDs. WARNING!!!
