...
foreach (@dirs) {
    my $item = $dir . "/" . $_;
    if (-T $item && $_ =~/^Project_\w+_(.*)_.*/i) { # csv log file
        $softRev = $1;
        #say "SoftRev: $1";
    }
}
...
foreach my $line (@fileContent){
    ...
    if ($line =~/^[\t|\s]+E/i) { # with E notation is Event line
        ...
        if ($ceidDecVal == 81936) { #Needs formula to calculate RCnLimitAIp(0-63)
            ...
            next;
        }elsif($ceidDecVal == 78081){ #Needs formula to calculate DCMnLimitAIp(0-95) for Synergis
            createCEIDseriesForRC(0, 95, "LimitAI", "second");
        }else{
