...
my $tempfile = ".\\Info\\ASM-Host.log"; #".\\Info\\S1F22.txt"; # We can parse file with only content of S1F12

if (!-e $tempfile) {
    say "$tempfile does not exist!";
    $tempfile = ".\\Info\\ASM-Host.log"; # or we can parse for the log file contains content of S1F12
    if (!-e $tempfile) {
        die "$tempfile does not exist!";
    }else {
        extractSECSMessageFromLog("S1F12");
    }
}else{
...
sub createOutFile{
    my $outFile = ".\\Info\\VIDsOnlyFromS1F12.txt";
    say("outFile: ", $outFile);
