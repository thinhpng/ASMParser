    ...
    if ($line_str =~/<L\[3\/1\]/i) {
        ...
        if($begin == 1){
            ...
            if ($line_str =~/<A\[\d+\/1\]\s"(.*?)"/i){
                ...
                $hexVid =~s/0x/\$0/;
                push(@VIDLines, "$type : $hexVid : $Vid : $Desc : \n");
                $vidCount++;
                print LOG "$type : $hexVid : $Vid : $Desc :\n";
            }
        }elsif ($begin > 2){
            $begin = 0;
            next;
...
sub createOutFile{
    $fileName = $type . "sOnlyFromHostLog.txt";
...
sub extractSECSMessageFromLog{
    ...
    foreach my $line_str (@tempBucket){
        ...
        #say "line_str=$line_str";
        chomp($line_str);
        if ($line_str =~/^.*?\s+<(S1F[1|2]2).*/ || $line_str =~/^.*?\s+<(S1F24).*/) { #Either S1F12 or S1F22 or S1F24
